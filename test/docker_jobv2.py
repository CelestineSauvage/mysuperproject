...
with DAG(
    ...
    schedule_interval=None, # Chron toutes les minutes
    catchup=False,
    tags=['docker']
)as dag:
    ...
    for source in data_source:
        ...
        t_extract = DockerOperator(
            ...
            mounts=[
                Mount(
                    source=f'{source_main_volume}/{data_folder_name}/{source_name}',
                    target=f'/project/downloads_from_airflow/{source_name}',
                    type='bind'),
                Mount(source=f'{source_main_volume}/{log_folder_name}', target='/project/logs', type='bind'),
                ],
