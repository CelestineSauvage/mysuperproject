...
class DataCollector:
    ...
    def collect():
        ...
        france_emploi_client_secret = os.environ.get("FRANCE_EMPLOI_CLIENT_SECRET", "")
        muse_client_secret = os.environ.get("MUSE_CLIENT_SECRET", "")
        if france_emploi_client_id == "":
            raise CredentialNotFoundException("france_emploi_client_id environment variable not found")
        if france_emploi_client_secret == "":
            raise CredentialNotFoundException("france_emploi_client_secret environment variable not found")
        if muse_client_secret == "":
            raise CredentialNotFoundException("muse_client_secret environment variable not found")
